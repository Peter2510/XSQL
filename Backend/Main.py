...
from src.visitor import ExpressionsVisitor, TablesValidVisitor
...
def compilar():
    if request.method == "POST":
        ...
        visitorTables = TablesValidVisitor(env)
        entrada = request.data.decode("utf-8")
        entrada = json.loads(entrada)
        pars = parse(entrada.lower())
        # Validaciones
        pars.accept(visitorExpressions, env)
        valid = visitorExpressions.correct
        if valid:
            pars.accept(visitorTables, env)
            valid = visitorTables.correct
        #
        if valid:
